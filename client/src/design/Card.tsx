...
const Card: React.FC<CardProps> = ({ id, title, text, T, F }) => {
  ...
  const [deleteCard] = useMutation(DELETE_CARD_MUTATION, {
    refetchQueries: [MYCARDS_QUERY, USER_AND_CARDS_QUERY],
  });
    ...
    <div>
      ...
      <div>
        <div className="flex p-2 m-2 border-4 pinkBorder rounded-md">
          <h1 className="m-2 text-lg md:text-2xl basis-4/6">{title}</h1>
          <button
            className="mr-2 md:text-xl text-white bg-black basis-2/6 rounded-md pinkHover"
            onClick={() => {
              navigate("/viewcard", { state: { id, title, text, T, F } });
            }}
          >
            <h1>View</h1>
